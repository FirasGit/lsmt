...
import numpy as np
from ..datasets.mimic_lab import CLASSES, R_CLASSES
...
class LightningClassifier(pl.LightningModule):
    ...
    def compute_loss(self, prediction, label):
        if isinstance(prediction, list):
            ...
            for prediction_ in prediction:
                loss += self.loss_fnc()(prediction_, label)
        else: 
            loss = self.loss_fnc()(prediction, label)
        return loss
    ...
    def _step(self, batch, batch_idx, prefix: str):
        ...
        if batch.get('label_names', False):
            self.label_names = batch.get('label_names')[0]
            prediction = self.get_prediction((img, ehr, age, gender, ethnicity))
            loss = self.compute_loss(prediction, targets_ehr)
            label = targets_ehr
        else:
            is_pretrain = self.cfg.model.meta.get('pretrain_cxr', {})
            is_pretrain = is_pretrain.get('is_true', False)
            if is_pretrain:
                self.label_names = R_CLASSES
                prediction = self.get_prediction((img, ehr, age, gender, ethnicity))
                loss = self.compute_loss(prediction, targets_cxr)
                label = targets_cxr
            else:
        ...
        if isinstance(prediction, list):
            prediction_prob = self._get_prediction_probability((2 * prediction[0] + 1 * prediction[1] + 1 * prediction[2]) / (4))
        else:
            prediction_prob = self._get_prediction_probability(prediction)
            

        self.log(f'{prefix}/Step/Loss', loss)
        return {'loss': loss, 'prediction': prediction_prob, 'label': label} 
