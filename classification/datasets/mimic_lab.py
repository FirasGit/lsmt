...
class MIMIC_CXR_EHR(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.dataset.data_pairs == 'paired_ehr_cxr':
            ...
            ehr_data, labels_ehr = np.zeros((1, 76)), np.zeros(self.cfg.model.output_logits)
...
def my_collate(batch):
    ...
    targets_ehr = torch.Tensor(np.array([item[2] for item in batch]))
...
def get_transforms(cfg):
    ...
    train_transforms = []
    train_transforms.append(transforms.Resize(cfg.model.transforms.img_size))
    train_transforms.append(transforms.RandomHorizontalFlip())
    train_transforms.append(transforms.RandomAffine(degrees=45, scale=(.85, 1.15), shear=0, translate=(0.15, 0.15)))
    train_transforms.append(transforms.CenterCrop(cfg.model.transforms.img_size))
    train_transforms.append(transforms.ToTensor())
    train_transforms.append(normalize)      


    test_transforms = []
    test_transforms.append(transforms.Resize(cfg.model.transforms.img_size))


    test_transforms.append(transforms.CenterCrop(cfg.model.transforms.img_size))
...
class EHRdataset(Dataset):
    def __init__(self, listfile, dataset_dir, cfg, return_names=True, period_length=48.0, split=None):
        ...
        self.discretizer_header = self.discretizer.transform(read_timeseries())[1].split(',')
        self.cont_channels = [i for (i, x) in enumerate(self.discretizer_header) if x.find("->") == -1]
    ...
    def __getitem__(self, index, time_bound=None):
        ...
        ys = ret["y"]
        names = ret["name"]
        if self.cfg.dataset.drop_channel:
            channel_to_drop = ret['header'].index(self.cfg.dataset.drop_channel)
            data[:, channel_to_drop] = [''] * len(data)

        data = self.discretizer.transform(data, end=ts)[0] 
        if (self.normalizer is not None):
            if self.split == 'train':
                add_noise = self.cfg.dataset.transforms.add_noise.is_true
            else:
                add_noise = False
            data = self.normalizer.transform(data, 
                                             add_noise=add_noise, 
                                             mean=self.cfg.dataset.transforms.add_noise.mean, 
                                             std=self.cfg.dataset.transforms.add_noise.std)
...
def get_datasets(cfg):
    train_ds = EHRdataset(f'{cfg.dataset.ehr_data_dir}/{cfg.dataset.task}/train_listfile.csv', os.path.join(cfg.dataset.ehr_data_dir, f'{cfg.dataset.task}/train'), cfg=cfg, split='train')
    val_ds = EHRdataset(f'{cfg.dataset.ehr_data_dir}/{cfg.dataset.task}/val_listfile.csv', os.path.join(cfg.dataset.ehr_data_dir, f'{cfg.dataset.task}/train'), cfg=cfg, split='val')
    test_ds = EHRdataset(f'{cfg.dataset.ehr_data_dir}/{cfg.dataset.task}/test_listfile.csv', os.path.join(cfg.dataset.ehr_data_dir, f'{cfg.dataset.task}/test'), cfg=cfg, split='test')
    return train_ds, val_ds, test_ds
